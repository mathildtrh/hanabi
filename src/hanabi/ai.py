...
class Human(AI):
    ...
    def play(self):
        ...
        for color in list(hanabi.deck.Color):
            if hanabi.deck.Card(color, 5) in game.discard_pile:
                dead_pile.append(color)
            for number in [4, 3, 2]:
                if game.discard_pile.count(hanabi.deck.Card(color, number)) == 2:
                    dead_pile.append(color)
            if game.discard_pile.count(hanabi.deck.Card(color, 1)) == 3:
                dead_pile.append(color)
        num_dead_pile = len(dead_pile)
        ...
        if game.blue_coins == 0 : #no more blue coins
            ...
            for (ind,card) in enumerate(game.current_hand.cards):
                if (ind,card.number) not in playable:
                    print ('Human would ragequit and discard:', "d"%ind)
                    return "d%d"%ind
                else:
        ...
        if action == 3:
            ...
            if num_dead_pile > 0:
                c = random.randint(1,num_dead_pile)
                clue = "c%s"%dead_pile[c]
                clue = clue[:2]
